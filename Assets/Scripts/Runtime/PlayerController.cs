...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleGrounding()
    {
        ...
        // Wall detection
        _isAgainstLeftWall = Physics.OverlapSphereNonAlloc(transform.position + new Vector3(-_wallCheckOffset, 0),
            _wallCheckRadius, _leftWall, _groundMask) > 0;
        _isAgainstRightWall = Physics.OverlapSphereNonAlloc(transform.position + new Vector3(_wallCheckOffset, 0),
    ...
    [SerializeField]
    private float _wallJumpLock = 0.25f;

    [SerializeField]
    private float _wallJumpMovementLerp = 5;
    ...
    private void HandleDashing()
    {
        if (Input.GetKeyDown(KeyCode.X) && !_hasDashed)
        {
            ...
            {
                _dashDir = _facingLeft ? Vector3.left : Vector3.right;
            }
